...
using namespace std;
// "15-150 d: abcdeadddddddddddddd",
// "1-3 b: cdefg",
// "100-500 c: ccccccccccc"
const char *datos[] = {
    "1-4 n: nnnnn",
    "5-7 z: qhcgzzz",
    "7-11 m: mmmmmmsmmmmm",
...
int main()
{
    // int tam = sizeof(datos[0])/sizeof(datos[2][0]);
    // cout << datos[2] << endl;
    // cout << strlen(datos[0]) << endl;
    // cout << strchr(datos[0],'a') << endl;

    // cout <<  << endl;

    int tam = sizeof(datos) / sizeof(datos[0]);
    int contraseniasCorrectas = 0;
    // cout << "cantidad contrasenias : " << tam << endl;
    ...
    for (int j = 0; j < tam; j++)
    {
        ...
        tamCadaFilaDatos = strlen(datos[j]);
        // cout << "tamanio : " << tamCadaFilaDatos << endl;

        for (int i = 0; i < tamCadaFilaDatos; i++)
        {
            // cout << datos[j][i];
            if (spaceCount == 0)
            {

                if (guionCount == 0 && datos[j][i] != '-')
                {
                    cantidadInicial = cantidadInicial + datos[j][i];
                }
                if (guionCount == 1 && datos[j][i] != '-')
                {
                    cantidadFinal = cantidadFinal + datos[j][i];
                }
                if (datos[j][i] == '-')
                {
            ...
            if (spaceCount == 1)
            {
                if (datos[j][i] != ':')
                {
                    letra = letra + datos[j][i];
                }
            }
            if (spaceCount == 2)
            {
                contrasenia = contrasenia + datos[j][i];
            }
            if (datos[j][i] == ' ')
            {
        ...
        // cout <<"inicial : "<< cantidadInicial << endl;
        // cout <<"final : "<< cantidadFinal << endl;
        // cout << letra[0] << endl;
        // cout << contrasenia << endl;

        int menor = atoi(cantidadInicial.c_str());
        int mayor = atoi(cantidadFinal.c_str());

        int countLetra = 0;
        // cout << endl << contrasenia.length() << endl;
        // for (int m = 0; m <= contrasenia.length(); m++)
        // {
        //     // cout << contrasenia[m];
        //     char letra1 = contrasenia[m];
        //     char letra2 = letra[0];
        //     if (letra1 == letra2)
        //     {
        //         countLetra = countLetra + 1;
        //     }
        // }

        char letra1 = contrasenia[menor - 1];
        char letra2 = letra[0];
        if (letra1 == letra2)
        {
            countLetra = countLetra + 1;
        ...
        // cout << countLetra << endl;

        if (countLetra == 1)
        {
            contraseniasCorrectas = contraseniasCorrectas + 1;
            // cout << endl << "correcta" << endl;
        }
        // else
        // {
        //     cout << endl << "incorrecta" << endl;
        // }
    }

    cout << "correctas: " << contraseniasCorrectas << endl;
